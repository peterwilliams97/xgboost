...
import os
import sys
# optionally have scipy sparse, though not necessary
import numpy
import numpy.ctypeslib
import scipy.sparse as scp

IS_WINDOWS = sys.platform == 'win32'
IS_64_BIT = ctypes.sizeof(ctypes.c_voidp) > 4
...
class DMatrix:
    ...
    def __init__(self, data=None, label=None, missing=0.0, weight=None):
        ...
        if isinstance(data, str):
            ...
            try:
                ...
                raise Exception('can not intialize DMatrix from %s' % type(data))
        if label != None:
    ...
    def __init_from_csr(self, csr):
        ...
        xglib.XGDMatrixParseCSR( self.handle,
                                 ( size_t * len(csr.indptr) )(*csr.indptr),
                                 ( size_t * len(csr.indices) )(*csr.indices),
    ...
    def get_label(self):
        #assert False # Should c_ulong() be size_t ctor 
        length = size_t() # ctypes.c_ulong()
...
class Booster:
    ...
    def __init__(self, params={}, cache=[]):
        """ constructor, param: """
        for d in cache:
            assert isinstance(d, DMatrix), type(d)
    ...
    def update(self, dtrain):
        """ update """
        assert isinstance(dtrain, DMatrix), type(dtrain)
